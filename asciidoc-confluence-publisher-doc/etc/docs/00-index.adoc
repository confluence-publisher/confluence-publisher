...
Publishing to a locally installed Confluence instance might require the use of REST API v1 (see <<restApiVersion,`restApiVersion`>> configuration property)
====
...
be used as password. Make sure to set the correct root URL for Confluence depending on the token type  (see <<rootConfluenceUrl,`rootConfluenceUrl`>> configuration property).
