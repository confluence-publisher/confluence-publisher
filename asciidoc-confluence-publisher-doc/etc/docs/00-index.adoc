...
The publisher will ignore any '.adoc' files which start with a '.' prefix. This is intended to ensure '.asciidoctorconfig.adoc' files will not be included in the files published to confluence.
...
be used as password. Make sure to set the correct root URL for Confluence depending on the token type.
