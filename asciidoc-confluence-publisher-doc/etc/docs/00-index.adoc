...
[NOTE]
====
The Confluence Publisher has been tested with Confluence 6.0.5 and Confluence Cloud.
...
| The version of the Confluence REST API to be used, either `v1` or `v2`. Default is version 2.
  +
  +
  _Note:_ version 1 is no longer supported for Confluence Cloud. Only use version 1 for on-premise Confluence and only in case of issues with version 2.
