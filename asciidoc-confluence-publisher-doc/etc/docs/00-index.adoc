...
| [[rootConfluenceUrl]] rootConfluenceUrl
| The root URL of the Confluence instance to publish to.

_Note:_ when publishing to Confluence Cloud using an API token, the root URL is `+https://<project>.atlassian.net/wiki+`.

_Note:_ when publishing to Confluence Cloud using a scoped API token, the root URL is `+https://api.atlassian.com/ex/confluence/<cloud-id>+` (see Atlassian documentation on how to identify the cloud id).
| mandatory
...
_Note:_ when publishing to Confluence Cloud, specify the email address of the corresponding Atlassian account (even when using an API token or a scoped API token as password).

_Note:_ when publishing to Confluence Server using a personal access token, the username must be left unspecified/empty.

_Note:_ overrules username defined by serverId, when set in conjunction with serverId


| optional (unspecified behaves like an empty string)

| password
| The password, API token or scoped API token of the user to use for publishing, or a personal access token when the username is unspecified/empty.

_Note:_ when publishing to Confluence Cloud, an API token or a scoped API token generated via the corresponding Atlassian account has to
be used as password. Make sure to set the correct root URL for Confluence depending on the token type  (see <<rootConfluenceUrl>> configuration property).

_Note_: when using a scoped API token, the following OAuth 2.0 scopes are required:
...
| The strategy to use when detecting orphan pages (pages in Confluence not / no longer part of the documentation source
...
_Note:_ Confluence only supports version messages for updated pages, but not when creating new pages.
...
| failOnError
| Defines whether the build should fail when an error occurs during publishing (e.g. when Confluence server is not
reachable or there are errors in provided pages/markdown). When set to `false`, errors are logged but the build
...
In addition to the Maven plugin and the Docker image, the Confluence Publisher can also be used as a standalone CLI tool. The CLI tool is available as a JAR artifact in the same version as the Maven plugin and can be executed with Java 11 or higher.

The CLI tool accepts the same configuration properties as the Maven plugin but they are used as command-line parameters.
Instead of downloading the JAR artifact manually, rather use JBang to directly execute the CLI tool with the following command:
...
jbang run --java 21 \
  -m org.sahli.asciidoc.confluence.publisher.cli.AsciidocConfluencePublisherCommandLineClient \
  org.sahli.asciidoc.confluence.publisher:asciidoc-confluence-publisher-cli:RELEASE \
  ...
  orphanRemoveStrategy=REMOVE_ORPHANS \
  convertOnly=false \
  asciidocRootFolder=./docs/
----
...
set CONFLUENCE_URL=https://....
set CONFLUENCE_SPACE=....
set CONFLUENCE_ANCESTOR_ID=....
